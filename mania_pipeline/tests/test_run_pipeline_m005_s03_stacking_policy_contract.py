...
def _load_module():
    spec = importlib.util.spec_from_file_location("run_pipeline_under_test_m005_s03", SCRIPT_PATH)
...
def _write_feature_csv(path: Path) -> Path:
    rows = []
    seed_diffs = [1.4, -1.3, 1.2, -1.1, 0.9, -0.8, 0.6, -0.5, 0.4, -0.3]
    net_diffs = [0.8, -0.7, 0.7, -0.6, 0.5, -0.4, 0.3, -0.2, 0.2, -0.1]
    for idx, (seed_diff, net_diff) in enumerate(zip(seed_diffs, net_diffs), start=1):
        if idx <= 4:
            split = "Train"
            season = 2022
        elif idx <= 7:
...
def _build_context(tmp_path: Path) -> dict:
    ...
    hpo_report_path = _write_json(
        ...
        {
            "run_id": "m005_s03_stacking_contract",
            "status": "passed",
...
def test_stage_eval_report_emits_stacking_policy_contract(tmp_path, monkeypatch):
    module = _load_module()
    ...
    class StubTrainModule:
        ...
        def train_baseline(df, gender="M", random_state=42, profile="baseline", param_overrides=None):
            ...
            bias = 0.7 if isinstance(param_overrides, dict) and param_overrides else -0.6
            model = ContractProbModel(features, bias=bias, importance=[1.0 for _ in features])
            base_brier = 0.20 if gender == "M" else 0.19
            adjust = -0.01 if isinstance(param_overrides, dict) and param_overrides else 0.0
            payload = {
                "gender": gender,
    ...
    payload = result["stacking_policy"]
    report_path = Path(payload["report_json"])
    assert report_path.exists()

    report = json.loads(report_path.read_text(encoding="utf-8"))
    assert report["policy_name"] == "stacking_policy_research_v1"
    assert set(report["by_gender"].keys()) == {"men", "women"}

    for gender_key in ("men", "women"):
        gender_payload = report["by_gender"][gender_key]
        assert "calibration_platt" in gender_payload["candidate_ids"]
        assert "calibration_isotonic" in gender_payload["candidate_ids"]
        assert "diversity" in gender_payload
        assert "val" in gender_payload["diversity"]
        assert "test" in gender_payload["diversity"]
        assert "stacking_candidate" in gender_payload
        assert gender_payload["stacking_candidate"]["candidate_id"] == "stacking_logreg"
        assert gender_payload["stacking_candidate"]["feature_count"] >= 2
        assert gender_payload["research_decision"] in {"promising", "not_promising", "insufficient_data"}

    eval_report = json.loads(Path(result["eval_report"]).read_text(encoding="utf-8"))
    assert eval_report["stacking_policy"]["report_json"] == str(report_path)
