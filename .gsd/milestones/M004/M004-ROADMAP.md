# M004: Performance Stabilization & Final Tuning

## Slices

- [x] **S01: Benchmark Lock + Evaluation Harness** `risk:medium` `depends:[]`
  > Başlangıç benchmark’ını kilitle ve M004 kıyas altyapısını kur.

- [x] **S02: CV-Strengthened HPO Selection** `risk:high` `depends:[S01]`
  > HPO seçim sinyalini daha güvenilir hale getir.

- [x] **S03: Ensemble Robustness Rules** `risk:medium` `depends:[S02]`
  > Ensemble kararını daha stabil ve genellenebilir yap.

- [x] **S04: Final Performance Proof + Freeze** `risk:medium` `depends:[S03]`
  > Önce/sonra kalite kanıtı + readiness doğrulaması + kapanış.
